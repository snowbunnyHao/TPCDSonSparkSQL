...
select
 ...
 from (select item,return_ratio,return_rank,currency_ratio,currency_rank
      from(select item
      ,return_ratio
      ,currency_ratio
      ,rank() over (order by return_ratio) as return_rank
      ...
      (     select cs.cs_item_sk as item
            ...
            cast(sum(coalesce(cs.ws_quantity,0)) as decimal(15,4) )) as return_ratio
            ...
            catalog_sales cs 
                ...
                left outer join catalog_returns cr
                  on (cs.ws_order_number = cr.cr_order_number and 
                  cs.cs_item_sk = cr.cr_item_sk)
            where 
                  cr.cr_return_amount > 10000 
                  and cs.cs_net_profit > 1
                         and cs.cs_net_paid > 0
                         and cs.ws_quantity > 0
                         and d_year = 2000
                         and d_moy = 12
                 group by cs.cs_item_sk
      ) in_cat
 ) a3 )catalog
 where  return_ratio <= 10 or currency_ratio <=10
 union all
 select 
 'store' as channel
 ,store.item myitem
