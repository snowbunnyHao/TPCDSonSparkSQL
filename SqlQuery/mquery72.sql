...
left outer join promotion pm on (cs.ws_promo_sk=pm.p_promo_sk)
left outer join catalog_returns cr on (cr.cr_item_sk = cs.cs_item_sk and cr.cr_order_number = cs.ws_order_number)
where x.inv_quantity_on_hand < cs.ws_quantity
